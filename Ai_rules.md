...
- **Every new function or module must have at least one test.** No exceptions for "simple" utilities.
- Tests must be in the project's established test framework (Jest, Pytest, Vitest, Go test, etc.) — do not introduce a second framework.
- Test structure: `Arrange → Act → Assert`. One assertion cluster per test case.
- Name tests descriptively: `it("returns 404 when user does not exist")` not `it("works")`.
- Mock external dependencies (network, DB, filesystem) in unit tests — tests must not make real network calls.
- Write at least one **unhappy path** test per function (invalid input, missing data, failure from a dependency).
- Do not write tests that test the mock itself — test behavior, not implementation.
- Run the full test suite before marking a task done. Do not leave failing tests.
- Keep test coverage of new code above 80%. Do not decrease overall coverage.
